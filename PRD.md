...
- **产品外壳已建成并被人手驱动过(2026-07-20)**:感知→规划→编辑→自检→收敛→接受/回滚全自动,
  可停靠面板、成本闸门、钥匙串、三屏仪表盘、41 条离线自测 + 6 条结构 lint。详见 §九。
...
4. **实例寻址补完**(KNOWN_ISSUES #2)—— `dump_comp` 出 parade 序号 + spec 加 `effectIndex` +
   planner 提示词,**三处一起改**。
5. **essence 广度按 dogfooding 日志补**,不盲目铺量。

*(上手:`./shell/shell_up.sh` 起全套 · `bridge_up.sh` 只起桥 · `node test/smoke.mjs` 离线自测(41 条,
