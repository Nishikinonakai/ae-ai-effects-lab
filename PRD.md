...
- ~~**文本内容编辑 op**(2026-07-21 用户破坏性测试暴露)~~ **已建成并真机验证(当日,日志 §G)**:
  `textContent` 整串替换 + 感知透出文字与关键帧标记;关键帧/表达式文本拒绝而非猜
- ~~**新建图层 op**(同上暴露)~~ **已建成并真机验证(当日,日志 §G)**:`addLayer` solid/adjustment,
  顶部插入 + 感知索引重映射 + 按 id 可逆(回滚连固态素材一起清,零孤儿)
- **配方重写**:licensing 决策已定(option b),**重写工作仍未做**
- **运行时泛化**:introspect-on-install 的产品化,**仍只在这台 AE 2022 + TC2023 英文 macOS 验证过**
  —— Phase C 最大风险
- **成本/延迟工程:缝已建,省钱未动**(2026-07-20 深夜):按用途模型路由的 seam 全链贯通
  (`modelFor(purpose)`+`models.json`,askJSON 与三 scorer 同源),**默认值刻意不变**——换判官
  模型 = 换量具,等 #1 标定。真正省钱的模型指派与帧/感知缓存仍未做
...
| Electron 窗口壳 | ✅ 双路径首跑(2026-07-20 深夜) | 复用/自启都验证;首跑抓到孤儿 kernel 泄漏并修掉(日志 §D.7) |
