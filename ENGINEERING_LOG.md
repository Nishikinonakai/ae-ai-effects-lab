...
## E · 2026-07-20 深夜续 —— dogfooding 排练:真实 Datte PV 上的三次发散实跑

> 用户提议:从真实老工程里挑一个中等复杂度的,复制一份,computer-use 看画面,由 agent 发散提想法、
> 用产品实跑。**意图不是用户提的,所以这不算 §C.7 的正式 dogfooding**——但它第一次把产品放进了
> 真实工程的复杂度里(50 层 comp、第三方深效果、嵌套预合成),而且全程走面板路径。纪律:同目录
> 副本(相对素材不断链)、只记录不修 bug、逐轮回滚、原件零接触、结束后逐参数验证还原。

### E.1 对象与方法

**Datte PV(2023-12,40MB / 123 comps / 2798 层 / 833 表达式)**——七工程里的中位复杂度,
静止画 MAD:红场单色调 + 装甲明朝歌词 + y2k UI 拼贴 + BCC/7in1 故障打点 + **「狂ってる」四连快切
收尾在 0.24s 的 Form 粒子爆散**(3018 参数)。computer-use 沿母带扫 6 个时间点 + 全分辨率渲帧
建立视觉语感;产品跑在 comp 03(50 层)。六个发散提案,按能力轴选跑三个。

### E.2 三次实跑

| | 意图(模糊感觉体) | 产品的方案 | 分 | 帧上真相 |
|---|---|---|---|---|
| A | 红场极缓"呼吸",像布下有光,克制到几乎察觉不到 | Wave Warp(高12/宽1200/速0.1)+ opacity `value-2+sin(t·1.2)·2` | **7** | 拼贴无损,Δ均值 3.1/255——正中"察觉不到"档,**可以 keep** |
| B | 稀疏竖直白雨丝,微光,与横向故障成十字张力 | FN(对比850/亮-320)→表达式驱动 Offset 下落→Transform ScaleY **2500** 拉丝→Glow→**Solid Composite 垫红**(知道 FN 会吃图层并主动补偿!) | **3** | 组合数值翻车:红场整个变灰白,雨丝无形。**结构惊艳,数值致命**——eval r2 结论 (b) 在真工程上现场复现 |
| C | Form 爆散残粒被风右扫,拖余烬尾巴,"消散得更留恋" | Form **Size Random→75(穿过 400 参数截断写中了深参数,别名表兑现)**+ Directional Blur + Turb. Displace + Glow | **5** | playhead 处 before 本就近空,后处理把残粒抹得更净。**时域意图被静默投影成静帧优化** |

三轮全部诚实交接(零假完成),三轮回滚全部干净(结束后逐参数验证:层 47 回到 1 效果 + 无表达式,
层 15 回到 Form+Glitch 7in1)。**安全故事在 50 层真实工程上成立。** 全程 LLM 开销 ≈ $0.15。

### E.3 三条新认识(按贵重排序)

1. **时域意图是一个未声明的盲区(C 与 A 共享)。** "消散得更留恋"、"起伏像布料"——判定它们需要
   看一段时间,而环只看 before/after 静帧。产品既没拒绝也没声明,把时域诉求静默投影到单帧上优化。
   诚实的产品应当**识别时域意图并说出自己判不了**(或渲多帧对)。已入 KNOWN_ISSUES #13。
2. **数值组合效应是当前平原的真实形态(B)。** 五效果链每个参数单看都合理,叠起来白茫一片;
   建议通道每轮只动几个杠杆,2 轮救不回 5 杠杆的连锁失误。这不是新缺陷,是 §四 A "planner 质量"
   的真工程测量——**结构力已够格,配平力是短板**。
3. **正面证据也要记:** 深效果编辑穿过感知截断(Form 别名表)、"克制"类模糊意图产出 keep 级结果、
   面板全程可视(spend 行 + what-it-changed)。产品在真实工程里**不吓人**——这句话第一次有实据。
...
## G · 2026-07-21 —— 路线抉择(核心先行)+ §F 点名的两个 op 落地

> 用户问:下一步打磨核心还是先做周边(如外壳一键装插件)?决定:**核心先行**——dogfooding
> 方法论的意义就是让真实使用决定修什么,而第一批真实数据(§F)明确指向编辑协议的两个缺口;
> 一键安装服务的是还不存在的新机器/新用户(Phase C 打包),在产品接不住用户请求前先修门面
> 是本末倒置。(当晚装面板被 root 属主 + TCC 折腾两轮,恰说明装置类要留给壳统一做。)

### G.1 编辑协议 3 op → 5 op

- **`textContent`**:TextDocument 取值→改 text→回写(字号/字距等样式随文档保留),整串替换;
  关键帧文本(逐 key 歌词切换是本体裁惯用法)与表达式驱动文本**拒绝而非猜**。感知端配套:
  `dump_comp` 对文字层输出 `text` 字段(截 60 字)+ `[KEYFRAMED×n]` 标记,让"把所有 X 换成 Y"
  变得可规划;校验器在规划期就拦下非文字层与关键帧层。
- **`addLayer`**(solid / adjustment):生成类请求获得自己的画布,不再把雪贴到歌词预合成上。
  **索引约定**:新层永远插到顶部,同 spec 后续编辑对旧层继续用感知期索引(applier 按已加层数
  重映射),新层用 `layerIndex: 0`(按 AE layer id 解析)。逆操作按 id 删层,**连它的固态素材项
  一起清**(usedIn 为空才删——回滚不许往素材箱里留孤儿)。
- **连带的正确性修复**:所有逆操作的 layerIndex 从"感知期索引"改为**应用时刻真实索引**
  (`L.index`)——spec 混入 addLayer 后感知索引就是陈旧的;按"逆序回滚"推演,某编辑的逆操作
  执行时其目标层的当前索引恰等于应用时刻索引,数学自洽。

### G.2 真机验证(冷启 AE、空工程、全部回滚后关闭)

综合 spec 一次覆盖:建层 → layerIndex 0 上加 Fractal Noise + 调参 → 旧文字层在 +1 顶移后仍被
正确改写("Datte forever"→"Claude forever" 落在顺移后的 3 号层)→ 关键帧文本层拒绝。回滚:
文字精确还原、效果移除、按 id 删层、素材箱零孤儿(numItems 逐项核对)。adjustment 变体
(建调整层 + Lumetri)同样全链验证。

**首次执行又抓出两个真 bug**(存在≠能跑,第 N 次兑现):①循环在分发前统一解析目标层,而
addLayer 没有 layerIndex,整个分支从未执行——改为按 op 决定是否解析;②**AE 22.6 没有
`comp.layerByID`**(后续版本 API;`layer.id` 有),按 id 解析静默抛错——改为手写 id 扫描,
全版本可用。后者同时是 KNOWN_ISSUES #8(跨版本 API 面漂移)的又一实证。

离线自测 78 → **87 条全绿**(textContent 角色/关键帧/空串拦截、addLayer 约定与 layerIndex 0
