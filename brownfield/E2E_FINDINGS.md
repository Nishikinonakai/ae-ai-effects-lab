...
| 3 | **Async `saveFrameToPng` not awaited** — the ExtendScript returns before the PNG finishes (a 4K frame takes ~12s). A half-written frame (bottom rows black) got scored **0/10 "catastrophically broken"** — a false negative that would derail the tune loop. | **Critical** | **FIXED across edit + perception paths** — `apply_edit`, temporal sampling, and `dump_comp` share `waitForFrameSettle`; completion now requires both stable size and terminal PNG `IEND`, because a mid-write size plateau was reproduced live and then failed zlib with `unexpected end of file`. |
