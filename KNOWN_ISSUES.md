...
>
> 最后核对:2026-07-20 深夜(#2 #4 退役;新增 #13 时域盲区;桥自愈落地——AE 重启杀桥曾让 21:09 一次真实请求
...
### 9. 成本 / 延迟工程 —— 缝已建,省钱仍未发生

PRD §10.3 起就排在第 4 位。现在每轮 review 一次 vision 调用,tune 最多 4 轮。

**2026-07-20 深夜落了第一片:按用途路由的缝**(`shell/llm.mjs::modelFor(purpose)` +
`~/Documents/ae-ai-shell/models.json`,优先级 显式 flag > 配置 > provider 默认)。askJSON 与三个
scorer 的默认值全部改走这一条缝,scorer 里的硬编码模型名同步退役。**没写配置时行为逐位不变**
——这是刻意的:换判官模型 = 换量具(C.2 教训),要等及格线标定(#1)有了标注集再动。

**仍未做的才是省钱本体:** ① 真的给 plan 配便宜模型/给 vision 配强模型(需要标定支撑);
② 帧与感知结果缓存。**这条如果早做,2026-07-20 那天会便宜一个数量级。**

计量与闸门已经有了(`shell/llm.mjs` 台账 + `shell/budget.mjs` 上限,含 per-request 窗口),
**但"少花钱"和"不超支"是两件事,现在只做了后者加一条空缝。**
